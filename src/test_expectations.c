...
void expectFalse(bool a, const char *msg) {
  ...
  testFailure = true;
  printf(RED "Expected %s to be false but was %d\r\n" RESET, msg, (int)a);
}
...
void expectTrue(bool a, const char *msg) {
  ...
  testFailure = true;
  printf(RED "Expected %s to be true but was %d\r\n" RESET, msg, (int)a);
}
...
void expectEqualBytes(byte a, byte b, const char *msg) {
  ...
  testFailure = true;
  printf(RED "Expected %s to equal %d, but was %d\r\n" RESET, msg, b, a);
}
...
void expectEqualChars(char a, char b, const char *msg) {
  ...
  testFailure = true;
  printf(RED "Expected %s to equal '%c', but was '%c'\r\n" RESET, msg, b, a);
}
...
void expectEqualPtrs(uint16_t *a, uint16_t *b, const char *msg) {
  ...
  testFailure = true;
  printf(RED "Expected %s to equal %p, but was %p\r\n" RESET, msg, b, a);
}
...
void expectEqualEscapedString(const char *a, const char *b) {
  ...
  printf(RED "Expected output of \"%s\" but got \"%s\"\r\n" RESET, b, a);
}
